...
/*NRF24L01指令定义区*/
#define NRF24L01_R_REGISTER			0x00	//读寄存器，高3位为指令码，低5位为寄存器地址，后跟1~5字节读数据
#define NRF24L01_W_REGISTER			0x20	//写寄存器，高3位为指令码，低5位为寄存器地址，后跟1~5字节写数据
#define NRF24L01_R_RX_PAYLOAD		0x61	//读Rx有效载荷，后跟1~32字节读数据
#define NRF24L01_W_TX_PAYLOAD		0xA0	//写Tx有效载荷，后跟1~32字节写数据
...
#define NRF24L01_R_RX_PL_WID		0x60	//读取Rx FIFO最前面一个数据包的长度，后续跟1字节读数据，仅适用于动态包长模式
#define NRF24L01_W_ACK_PAYLOAD		0xA8	//写应答有效载荷，高5位为指令码，低3位为通道号，后跟1~32字节写数据，仅适用于应答有效载荷模式
#define NRF24L01_W_TX_PAYLOAD_NOACK	0xB0	//写Tx有效载荷，不要求应答，后跟1~32字节写数据，仅适用于不要求应答模式
#define NRF24L01_NOP				0xFF	//空操作，单独指令，可以用来读取状态寄存器
...
#define NRF24L01_SETUP_RETR			0x04	//设置自动重发，1字节
...
#define NRF24L01_RX_PW_P1			0x12	//接收通道1有效载荷的数据长度，1字节
#define NRF24L01_RX_PW_P2			0x13	//接收通道2有效载荷的数据长度，1字节
#define NRF24L01_RX_PW_P3			0x14	//接收通道3有效载荷的数据长度，1字节
#define NRF24L01_RX_PW_P4			0x15	//接收通道4有效载荷的数据长度，1字节
#define NRF24L01_RX_PW_P5			0x16	//接收通道5有效载荷的数据长度，1字节

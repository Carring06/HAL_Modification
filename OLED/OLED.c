/***************************************************************************************
  * 本程序由江协科技为基础进行创建并免费开源共享
  * 适配HAL库版本，基于STM32F103C8T6
  * 原代码适配标准库，此处修改为HAL库兼容
  ***************************************************************************************
  */
...
/**
  ...
  * 返 回 值：无
  * 说    明：适配HAL库，修改为HAL_GPIO_WritePin操作
  */
...
/*
(1)	如果你的 CubeMX 里 PB8/PB9 确实只有 Alternate Function Open Drain 这一个选项，那说明这个引脚的硬件复用功能被强制绑定了，
	这会导致I2C通信无法正常工作我们可以用代码层面强制重映射的方式来解决，不需要依赖 CubeMX 的图形界面。
(2) CubeMX 的图形界面只是辅助配置工具，真正决定引脚行为的是寄存器。
	上面的代码直接操作寄存器，强制将 PB8/PB9 从 “复用功能” 切换到 “通用开漏输出”，绕过了 CubeMX 的限制。
	这样你的 OLED_W_SCL 和 OLED_W_SDA 函数就能正常控制引脚电平，软件模拟 I2C 就能跑起来了。
*/

/**

	@brief 当CubeMX因一定原因无法使I2C的SDA,SCL配置为通用开漏输出时，在I2C初始化后，调用此函数
	@param  无
	@return 无
*/
...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：CubeMX已完成引脚硬件初始化，此处仅保留延时和引脚释放逻辑
  */
void OLED_GPIO_Init(void)
{
	...
	/*CubeMX已完成GPIO初始化，此处仅释放SCL和SDA为高电平*/
	OLED_W_SCL(1);
	OLED_W_SDA(1);
}
